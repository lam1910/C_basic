...
//delete at cur position (have prev already, careful cur = root) (second ver no prev) (for delete at n position, will have another function)
void deleteMiddle()
{
  if (cur == root)
    deleteHead();
...
node_addr* deleteAddress(node_addr* root, address person)
{
}
node_addr* listReverse(addr_node* root)
{
  node *cur, *prev;
  cur = prev = NULL;
  while (root != NULL)
